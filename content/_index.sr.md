---
title: ""
---

# LibreSCRS — Libre Smart Card Reader System

Слободни алати за смарт картице за Linux и macOS.

`ISO 7816 · PC/SC · PKCS#11 · PKCS#15 · ICAO 9303 · NIST PIV · BSI TR-03110`

{{< button href="/sr/user-guide/" >}}Почните овде{{< /button >}}
{{< button href="/sr/developer-guide/" >}}За програмере{{< /button >}}

---

### Читајте било коју картицу

Графички читач смарт картица са подршком за плагинове. Чита српску личну карту, здравствену, саобраћајну (ЕУ) и е-пасоше; потпуна PKI подршка преко OpenSC-а.

### Проширите плагиновима

Направите middleware и GUI плагинове за нове типове картица. Архитектура заснована на стандардима, писана у савременом C++-у (C++23 middleware, C++20 GUI).

---

## Подржите LibreSCRS

Помозите да алати за смарт картице остану бесплатни и отворени.

{{< button href="/sr/donate/" >}}Подржите пројекат{{< /button >}}

---

## Лиценца

- **LibreCelik**: [GPL-3.0-or-later](https://www.gnu.org/licenses/gpl-3.0.html)
- **LibreMiddleware**: [LGPL-2.1-or-later](https://www.gnu.org/licenses/old-licenses/lgpl-2.1.html)
