...
LibreSCRS је изграђен око једног брокера: кросплатформског агента који поседује картицу, тајне и конфигурацију. Брокер учитава мотор — LibreMiddleware — у сопственом процесу. Свака друга површина је танак клијент који разговара са брокером и никада сам не дира картицу. На Linux-у брокер користи D-Bus и systemd. macOS позадина (App-Group сокет и launchd) је на мапи пута.

```
                       Смарт картице
   српска eID · PKS · здравство · возила · е-пасош · PIV · OpenPGP · …
                            │
                    PC/SC читачи (pcscd)
                            │
   Брокер — један кросплатформски агент: поседује картицу, тајне, конфигурацију
   (Linux: D-Bus + systemd · macOS: App-Group сокет + launchd)
     учитава LibreMiddleware (мотор):
       PKI  — opensc → OpenSC + pkcs15
       Data — rs-eid · rs-health · eu-vrc · emrtd
                            │ опслужује клијенте (сви без LibreMiddleware-а)
   LibreCelik · LibreKDE · LibreMac (хост + CTK екстензија) · стандардне PKCS#11 апликације
```

Преглед површина данас:

- **LibreCelik** (Qt6 GUI) је клијент који се тренутно испоручује; ради на Linux позадини.
- **LibreKDE** (KDE Plasma) је следећи Linux клијент и налази се на мапи пута.
- **LibreMac** спаја SwiftUI хост са танком CryptoTokenKit токен екстензијом; његова macOS позадина је на мапи пута и покренута је на стварном хардверу.
- **Стандардне PKCS#11 апликације** долазе до картице кроз агентов PKCS#11 клијент. Постоји и директан PKCS#11 модул линкован са LibreMiddleware-ом за headless употребу.

## Компоненте

### LibreMiddleware (LGPL-2.1)

Колекција статичких C++23 библиотека за комуникацију са смарт картицама, без зависности од Qt-а. Обрађује све од APDU команди ниског нивоа до екстракције података са картице. Брокер га учитава у свом процесу и поседује сву PC/SC комуникацију; клијенти никада директно не дирају картицу.
...
|---|---|---|
| `LibreSCRS::SmartCard` | `LibreSCRS::SmartCard` | `CardSession`, `MonitorService` (API за снимак листе читача од 4.2), `CardMap` — PC/SC + монитор |
| `LibreSCRS::Plugin` | `LibreSCRS::Plugin` | `CardPlugin`, `CardPluginService`, `CardData`, `CardDataAccess` (`textValue` / `textValueAt`, од 4.2), `ReadResult` |
...
| `pkcs11` | PKCS#11 дељена библиотека (`librescrs-pkcs11`) — излаже PKCS#15 / OpenSC-везане породице картица (види Подржане картице) као PKCS#11 токене |
...
Јавна корисничка површина живи испод `LibreSCRS::` корена у
PascalCase именским просторима. Свако заглавље испод `include/LibreSCRS/`
припада овој површини; сви други простори су интерни.

| Простор имена | Опсег |
|---|---|
| `LibreSCRS::SmartCard` | `CardSession`, `MonitorService` (`subscribeReaderList` снимак, од 4.2), `ActiveChannelHolder`, `SmProtocolRequest`, `CardMap` — PC/SC + монитор догађаја картице + активација безбедних канала |
| `LibreSCRS::Plugin` | `CardPlugin`, `CardPluginService`, `CardData`, `CardDataAccess` (`textValue` / `textValueAt`, од 4.2), `ReadResult`, `AutoReaderService` |
