...
LibreMiddleware испоручује C++23 SDK под `LibreSCRS::*` umbrella-ом. Површина је намерно уска — шест простора имена, ~28 јавних хедера — и стабилна је кроз 4.x линију по [API политици]({{< ref "developer-guide/sdk-reference/api-policy" >}}).
...
- `TrustStoreService` — власник животног циклуса. **Асинхрона, не-throwing factory.** `create(TrustConfig)` враћа `std::expected<TrustStoreService, CreateError>` и при успеху одмах даје сервис са bundled + system анкерима; eager TL преузимања раде на интерним worker нитима и спајају се у јавни store како завршавају. Потрошачи посматрају завршетак кроз `status()`, `addObserver(...)`, или opt-in `waitForEagerFetches(deadline, CancelToken token = {})`.
...
**Plugin ABI:** v5 (3.x) → **v7** (4.0). In-tree додаци МОРАЈУ бити поновно компилирани против 4.0 хедера. `static_assert` макроа `LIBRESCRS_DECLARE_CARD_PLUGIN(T, 7)` хвата mismatch верзије у compile-time.
