...
- **Дељење сесије унутар процеса замењује attach C ABI** — 4.1
  ...
  Поседовање `CardSession`-а као `std::shared_ptr` сада се аутоматски
  координира кроз интерни `SessionPresence` регистар, па путања
  потписивања унутар процеса поново користи живу сесију безбедне размене
  порука хоста без експлицитног attach позива.
...
- **Очвршћавање конкурентности** — површина `MonitorService` / `Monitor`
  добија детерминистичку регресиону покривеност без хардвера (трка
  иницијалног окидања, животни циклус претплате/сажимач, гашење нити за
  испитивање, заштита од реентрантности `CardSession`-а).
